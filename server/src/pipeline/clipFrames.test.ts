...
describe('ClipSchema 프레임 하위호환', () => {
    ...
    expect(clip.frames).toEqual([
      {
        ...
        t: 0,
        recommended: true, // 옛 프레임은 전부 표시 대상
      },
    ]);
  });
  ...
  it('새 객체 형식은 그대로 통과한다', () => {
    const clip = ClipSchema.parse({
      id: 'c01',
      sourceId: 's01',
      frames: [{ file: 'a/frame_01.jpg', t: 1.5, recommended: true }],
    });
    expect(clip.frames[0]).toEqual({ file: 'a/frame_01.jpg', t: 1.5, recommended: true });
  });
  ...
  it('t·recommended는 생략 가능', () => {
    const clip = ClipSchema.parse({
      id: 'c01', sourceId: 's01', frames: [{ file: 'a.jpg' }],
    });
    expect(clip.frames[0]).toEqual({ file: 'a.jpg', t: 0, recommended: false });
  });
});
